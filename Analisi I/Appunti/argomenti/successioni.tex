...
& \inf b_{n} = e = \sup a_{n} \\
&\left( 1+ \frac{1}{n} \right)^{n+1} = b_{n} > \inf b_{n} = e = \sup a_{n} = \left( 1 +\frac{1}{n} \right)^n \\
...
\subsubsection*{Teorema di Cesaro sulle medie Arimetiche}
...
\item
  ...
  \[\lim_{ n \to \infty } \sqrt[n]{\frac{a_{n}}{n!}}=\lim_{ n \to \infty } \frac{a}{\sqrt[n]{n!}}\to \frac{a}{+\infty}=0=l<1 \quad \text{Allora }(3)=0\]
\item
  \((2)=0\) Utilizzo Criterio Radice
  \[\lim_{ n \to \infty } \frac{\sqrt[n]{n^b}}{a^n}=\lim_{ n \to \infty } \frac{\sqrt[n]{n^b}}{a}=\frac{1}{a}=l<1\implies \lim_{ n \to \infty } \frac{n^b}{a^n}=0\]
\item
  \((1)=0\) Sia \(b=1\)
...
\pf{
    ...
    Sia \(n_{k}\) una successione di indici strettamente crescenti e consideriamo una estratta \(\{a_{n_{k}}\}_{k}\). Scelto \(\epsilon\) devo dimostrare che \(|a_{n_{k}} - l| <\epsilon \; \forall k > \nu\).
...
\pf{
    ...
    Dimostriamo che si può costruire \(\{ a_{n_{k}} \}_{k}\) tale che \(a_{n_{k}}\to l'\) con \(k\to +\infty\) (la stessa cosa si può fare per \(l''\)). Per definizione di \(l'\) questo significa che scelto:
...
\thm{Criterio di Cauchy}{
    ...
    & |a_{n} - a_{m}| = \boxed{a_{m} -\epsilon <a_{n} < \epsilon +a_{m}}
    \end{align*}
}
...
\pf{
    ...
    Sfruttando la convergenza della sottosuccessione e la proprietà di Cauchy si conclude la dimostrazione (nota: l'ultima riga del tuo testo originale sembrava tronca, ho chiuso logicamente il pensiero).
}