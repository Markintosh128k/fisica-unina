...
\defn{Funzione}{
Siano $A, B \ne \varnothing$. Si chiama \textbf{funzione} $f: A \rarr B$ una legge che associa \textbf{ad ogni elemento di $A$ uno ed un solo elemento di $B$}.
...
\begin{itemize}
    ...
    \]
    Questo insieme può avere, a seconda dei casi, nessuno ($\varnothing$), uno o più elementi.
    \item L'insieme
    \[
...
Siano $A, B, C \ne \varnothing$ e siano $f: A \to B$ e $g: B \to C$.
...
\begin{enumerate}
    ...
    \end{align*}
  \end{enumerate}
\subsection{Funzione potenza (con esponente in \texorpdfstring{$\mathbb{N}$}{N})}
...
\subsubsection{Caso \texorpdfstring{$n$}{n} dispari}
...
\begin{itemize}
\item Se $\alpha > 1$, la funzione è \textbf{strettamente crescente} e \textbf{convessa}.
\item Se $0 < \alpha < 1$, la funzione è \textbf{strettamente crescente} e \textbf{concava}.
...
\subsection{Funzione radice \texorpdfstring{$n$}{n}-esima}
...
\begin{figure}[H]
    ...
    \caption{Grafici delle funzioni \(y=\sin(x)\) (blu) e \(y=\cos(x)\) (verde).}
...
\begin{figure}[H]
    ...
    \caption{Grafici delle funzioni \(y=\arcsin(x)\) (blu) e \(y=\arccos(x)\) (verde).}
