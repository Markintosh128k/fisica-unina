...
\rmkb{
    ...
    \textbf{Controesempi:}
    \begin{itemize}
        \item (1) \(K=[0,1]\), \(f(x)\) discontinua (es. vale 0 in 1, x in ]0,1[, 1 in 0). Non è continua ma ha max e min.
        \item (2) \(K=[0,1[\). \(1\) è un \(\sup\) ma non un \(\max\).
        \item (3) \(K=[0,1]\), funzione discontinua che non raggiunge gli estremi.
    \end{itemize}
    % [Immagini: percentuale-1.jpg, percentuale-2.jpg, percentuale-3.jpg]
}
...
\ex{
    ...
    % [Immagine: graf-asintoti-vert.jpg]
...
\ex{
    ...
    % [Immagine: graf-as-vert-oriz.jpg]
}
